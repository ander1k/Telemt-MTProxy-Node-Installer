...
apt_lock_pids() {
    local lock
    # Проверяем только процессы, действительно удерживающие lock-файлы.
    # unattended-upgrade-shutdown --wait-for-sig может работать постоянно,
    # ожидая выключения сервера, и сам по себе не означает занятый APT/dpkg.
    command -v fuser >/dev/null 2>&1 || return 0
    for lock in /var/lib/dpkg/lock-frontend /var/lib/dpkg/lock /var/lib/apt/lists/lock /var/cache/apt/archives/lock; do
        [[ -e "$lock" ]] && fuser "$lock" 2>/dev/null || true
    done | tr ' ' '\n' | sed '/^$/d' | sort -nu
}
wait_for_apt() {
    local started now elapsed remaining pid summary waited=false
    local -a pids=()
    started=$(date +%s)
    while true; do
        mapfile -t pids < <(apt_lock_pids)
        if [[ ${#pids[@]} -eq 0 ]]; then
            [[ "$waited" == true ]] && ok "APT/dpkg освободился — продолжаю установку"
            return 0
        fi
        waited=true
        now=$(date +%s); elapsed=$((now - started)); remaining=$((APT_LOCK_WAIT_SECONDS - elapsed))
        if (( remaining <= 0 )); then
            warn "APT/dpkg не освободился за ${APT_LOCK_WAIT_SECONDS} сек."
            ps -o pid=,etime=,comm=,args= -p "$(IFS=,; echo "${pids[*]}")" 2>/dev/null || true
            die "Дождитесь завершения системного обновления и повторите установку"
        fi
        summary=""
        for pid in "${pids[@]}"; do
            [[ -r "/proc/$pid/comm" ]] && summary+="${summary:+, }PID $pid ($(tr -d '\n' < "/proc/$pid/comm"))"
        done
        info "APT/dpkg занят: ${summary:-обновление системы}; ожидаю освобождения, максимум ещё ${remaining} сек."
        sleep 5
    done
}
apt_get() {
...
step "Плановый перезапуск Telemt"
hint "Перезапуск освобождает накопленные соединения; активные клиенты кратковременно переподключатся."
option 1 "Каждые 30 минут" "значение по умолчанию"
option 2 "Каждый час"
option 3 "Каждые 3 часа"
option 4 "Каждые 6 часов"
option 5 "Каждые 12 часов"
option 6 "Каждые 24 часа"
option 0 "Не требуется" "автоматический перезапуск выключен"
read -rp "$(echo -e "${YELLOW}Интервал планового перезапуска [1]: ${NC}")" AUTO_RESTART_CHOICE
AUTO_RESTART_CHOICE=${AUTO_RESTART_CHOICE:-1}
case "$AUTO_RESTART_CHOICE" in
    1) AUTO_RESTART_ENABLE=true; AUTO_RESTART_INTERVAL=30min; AUTO_RESTART_LABEL="30 минут" ;;
    2) AUTO_RESTART_ENABLE=true; AUTO_RESTART_INTERVAL=1h; AUTO_RESTART_LABEL="1 час" ;;
    3) AUTO_RESTART_ENABLE=true; AUTO_RESTART_INTERVAL=3h; AUTO_RESTART_LABEL="3 часа" ;;
    4) AUTO_RESTART_ENABLE=true; AUTO_RESTART_INTERVAL=6h; AUTO_RESTART_LABEL="6 часов" ;;
    5) AUTO_RESTART_ENABLE=true; AUTO_RESTART_INTERVAL=12h; AUTO_RESTART_LABEL="12 часов" ;;
    6) AUTO_RESTART_ENABLE=true; AUTO_RESTART_INTERVAL=24h; AUTO_RESTART_LABEL="24 часа" ;;
    0) AUTO_RESTART_ENABLE=false; AUTO_RESTART_INTERVAL=disabled; AUTO_RESTART_LABEL="не требуется" ;;
    *) die "Неизвестный интервал планового перезапуска" ;;
esac
result_begin "плановый перезапуск"
result_line "Состояние" "$([[ "$AUTO_RESTART_ENABLE" == true ]] && echo "включён" || echo "выключен")"
result_line "Интервал" "$AUTO_RESTART_LABEL"
...
if [[ "$PKG_FAMILY" == apt ]]; then
    ...
    if ! command -v docker >/dev/null 2>&1 || ! docker compose version >/dev/null 2>&1; then
        ...
        DEBIAN_FRONTEND=noninteractive apt_get install -y -qq docker-compose-plugin >/dev/null 2>&1 || true
    fi
    if ! command -v docker >/dev/null 2>&1 || ! docker compose version >/dev/null 2>&1; then
...
if [[ "$STUB_ENABLE" == true ]]; then
    if [[ "$PKG_FAMILY" == apt ]]; then DEBIAN_FRONTEND=noninteractive apt_get install -y -qq certbot >/dev/null
...
if [[ "$FAIL2BAN_ENABLE" == true ]]; then
    if [[ "$PKG_FAMILY" == apt ]]; then DEBIAN_FRONTEND=noninteractive apt_get install -y -qq fail2ban >/dev/null
...
step "Таймер перезапуска Telemt"
if [[ "$AUTO_RESTART_ENABLE" == true ]]; then
    # При повторной установке старый таймер не должен вмешаться в настройку.
    systemctl stop telemt-auto-restart.timer telemt-auto-restart.service >/dev/null 2>&1 || true
    cat > "$INSTALL_ROOT/bin/scheduled-restart.sh" <<'RESTARTSCRIPT'
#!/usr/bin/env bash
set -Eeuo pipefail
ROOT=/opt/telemt
COMPOSE=(docker compose --env-file "$ROOT/.env" -f "$ROOT/docker-compose.yml")
exec 9>/run/telemt-maintenance.lock
if ! flock -n 9; then
    echo "Другой процесс обслуживания Telemt уже выполняется — плановый перезапуск пропущен"
    exit 0
fi
if [[ $(docker inspect -f '{{.State.Running}}' telemt 2>/dev/null || true) != true ]]; then
    echo "Контейнер Telemt остановлен администратором — плановый перезапуск пропущен"
    exit 0
fi
echo "Плановый перезапуск Telemt начат: $(date --iso-8601=seconds)"
"${COMPOSE[@]}" restart --no-deps telemt
for _ in {1..30}; do
    status=$(docker inspect -f '{{.State.Health.Status}}' telemt 2>/dev/null || true)
    if [[ "$status" == healthy ]]; then
        echo "Плановый перезапуск завершён, Telemt healthy: $(date --iso-8601=seconds)"
        exit 0
    fi
    sleep 2
done
"${COMPOSE[@]}" logs --tail 80 telemt || true
echo "Telemt не прошёл healthcheck после планового перезапуска" >&2
exit 1
RESTARTSCRIPT
    chmod 700 "$INSTALL_ROOT/bin/scheduled-restart.sh"
    cat > /etc/systemd/system/telemt-auto-restart.service <<'UNIT'
[Unit]
Description=Scheduled Telemt restart with healthcheck
After=docker.service network-online.target
Requires=docker.service

[Service]
Type=oneshot
ExecStart=/opt/telemt/bin/scheduled-restart.sh
TimeoutStartSec=90
UNIT
    cat > /etc/systemd/system/telemt-auto-restart.timer <<EOF
[Unit]
Description=Restart Telemt every ${AUTO_RESTART_LABEL}

[Timer]
OnActiveSec=${AUTO_RESTART_INTERVAL}
OnUnitActiveSec=${AUTO_RESTART_INTERVAL}
AccuracySec=1s
Unit=telemt-auto-restart.service

[Install]
WantedBy=timers.target
EOF
    chmod 644 /etc/systemd/system/telemt-auto-restart.service /etc/systemd/system/telemt-auto-restart.timer
    systemctl daemon-reload
    systemctl enable telemt-auto-restart.timer >/dev/null
    ok "Плановый перезапуск настроен: каждые $AUTO_RESTART_LABEL; отсчёт начнётся после запуска Telemt"
else
    systemctl disable --now telemt-auto-restart.timer telemt-auto-restart.service >/dev/null 2>&1 || true
    for unit in /etc/systemd/system/telemt-auto-restart.service /etc/systemd/system/telemt-auto-restart.timer; do [[ -e "$unit" || -L "$unit" ]] && unlink "$unit"; done
    [[ -e "$INSTALL_ROOT/bin/scheduled-restart.sh" ]] && unlink "$INSTALL_ROOT/bin/scheduled-restart.sh"
    systemctl daemon-reload
    ok "Плановый перезапуск отключён"
fi
...
maintenance_lock() {
    exec 9>/run/telemt-maintenance.lock
    flock -w 30 9 || { echo "Другой процесс обслуживания Telemt ещё выполняется; повторите команду позже" >&2; exit 1; }
}
backup() {
...
case "${1:-}" in
  ...
  restart-schedule)
    if [[ "$AUTO_RESTART_ENABLE" == true ]]; then
      ...
      systemctl list-timers telemt-auto-restart.timer --no-pager
    else
...
if [[ "$AUTO_RESTART_ENABLE" == true ]]; then
    ...
    ok "Таймер запущен: следующий перезапуск через $AUTO_RESTART_LABEL"
fi
