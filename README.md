...
```bash
git clone https://github.com/ander1k/Telemt-MTProxy-Node-Installer/ telemt
cd telemt
sudo bash install.sh
```

Если Ubuntu/Debian в этот момент выполняет `unattended-upgrades`, мастер не удаляет lock-файлы и не останавливает обновление. Он показывает PID владельца APT/dpkg lock и безопасно ждёт до 15 минут. Лимит можно изменить только для конкретного запуска: `sudo APT_LOCK_WAIT_SECONDS=1800 bash install.sh`.
